	...
	<li component="category/topic" class="category-item hover-parent border-bottom py-3 py-lg-4 d-flex flex-column flex-lg-row align-items-start {function.generateTopicClass}" <!-- IMPORT partials/data/category.tpl -->>
		...
		<div class="d-flex p-0 col-12 col-lg-7 gap-2 gap-lg-3 pe-1 align-items-start {{{ if config.theme.mobileTopicTeasers }}}mb-2 mb-lg-0{{{ end }}}">
			<div class="flex-shrink-0 position-relative">
				<a class="d-inline-block text-decoration-none avatar-tooltip position-relative" title="{./user.displayname}" href="{{{ if ./user.userslug }}}{config.relative_path}/user/{./user.userslug}{{{ else }}}#{{{ end }}}">
					{buildAvatar(./user, "40px", true)}
					
					<!-- 国籍国旗开始 (Values: 缅甸, 越南, 新加坡) -->
					<span class="flag-overlay">
						<!-- IF ./user.customFields.nationality == "缅甸" -->🇲🇲<!-- ENDIF -->
						<!-- IF ./user.customFields.nationality == "越南" -->🇻🇳<!-- ENDIF -->
						<!-- IF ./user.customFields.nationality == "新加坡" -->🇸🇬<!-- ENDIF -->
					</span>
					<!-- 国籍国旗结束 -->
				</a>
				{{{ if showSelect }}}
			...
			<div class="flex-grow-1 d-flex flex-wrap gap-1 position-relative">
				<h3 component="topic/header" class="title text-break fs-5 fw-semibold m-0 tracking-tight w-100 {{{ if showSelect }}}me-4 me-lg-0{{{ end }}}">
					<a class="text-reset" href="{{{ if topics.noAnchor }}}#{{{ else }}}{config.relative_path}/topic/{./slug}{{{ if ./bookmark }}}/{./bookmark}{{{ end }}}">{./title}</a>
				</h3>
